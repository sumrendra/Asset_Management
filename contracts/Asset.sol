...
contract Asset is ERC721 {
    ...
    event TransferConditionSet(string previousCondition, uint256 indexed tokenId, string condition);
    ...
    function setTransferCondition(uint256 tokenId, string memory condition) public onlyPurchaseDepartment tokenExists(tokenId) {
        string memory previousCondition = assetTransferConditions.getAssetCondition(tokenId);
        assetTransferConditions.setAssetCondition(tokenId, condition);
        emit TransferConditionSet(previousCondition, tokenId, condition);
    }
